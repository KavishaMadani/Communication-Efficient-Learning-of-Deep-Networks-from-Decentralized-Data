...
os.makedirs(os.path.dirname(model_clusters_path), exist_ok=True)
...
def run_client(client_id):
  ...
  for round in range(args.rounds):
    ...
    precision = calculate_precision(local_model, testloader)
    recall = calculate_recall(local_model, testloader)
    f1_score = calculate_f1_score(precision, recall)

    with open(file_name + '_client_' + str(client_id) + '.csv', 'a', newline='') as csvfile:
      writer = csv.DictWriter(csvfile, fieldnames)
      writer.writerow({'round': round + 1, 'training_accuracy': train_accuracy, 'testing_accuracy': test_accuracy, 'precision': precision,
